...
Подсказка для пользователей Linux
===========================
Вы можете значительно ускорить поиск,
установив приложение на RAM-диск

Например, выполните от рута команды
```
mkdir /mnt/tmpfs
chmod 777 /mnt/tmpfs/
mount -t tmpfs -o size=1024M tmpfs /mnt/tmpfs/
```
Скопируйте файлы приложения (main-sqlite.db, magnetsdb, settings.ini, sql.txt,
tr.txt) в /mnt/tmpfs

Запустите magnetsdb из /mnt/tmpfs

Для удаления диска из памяти выполните с рут правами
```
unmount -v /mnt/tmpfs
```
